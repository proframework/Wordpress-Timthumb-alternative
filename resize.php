 ...
 * 
	...
	*  Additional tweaks (proportional resizing) by Paul Robinson (http://pbwd.co.uk + http://return-true.com)
	*  
 ...
 */
if ( isset( $wp_version ) && version_compare( $wp_version, '3.5' ) >= 0 && !function_exists('matthewruddy_image_resize') ) {
	function matthewruddy_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {
		...
		if(!$width && !$height)
			return new WP_Error( 'invalid_size', __( 'At least one of width or height must be a number.'), array($width, $height));
		...
		//If we aren't cropping - figure out the newly constrained dimentions here or we will jump out because of invalid width/height
		if(!$crop) {

			$sizes = wp_constrain_dimensions($orig_width, $orig_height, $dest_width, $dest_height);
		...
		if ( !file_exists( $dest_file_name ) ) {
			...
			if ( $crop ) {
				...
				//Oh wait? We aren't cropping? Then resize proportionally
...
elseif ( !function_exists('matthewruddy_image_resize') ) {
	function matthewruddy_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {
